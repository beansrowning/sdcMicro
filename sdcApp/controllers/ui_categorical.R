...
output$ui_recode <- renderUI({
  ...
  output$recfac_cbgr <- renderUI({
    cbgr <- selectInput("cbg_recfac",
      label = p("Select levels to recode/combine"), multiple = TRUE, selectize = TRUE,
  ...
  output$recfac_txtval <- renderUI({
    req(input$cbg_recfac)
    txtval <- textInput("inp_newlevname_rec",
      label = p("Specify new label for recoded values"),
...
output$ui_pram_expert <- renderUI({
  ...
  output$pram_expert_var <- renderUI({
    ...
    selectInput("sel_pramvars_expert",
      choices = pramVars(),
...
output$ui_pram_simple <- renderUI({
  output$pram_simple_var <- renderUI({
    ...
    selectInput("sel_pramvars_simple",
      choices = pramVars(),
  ...
  output$pram_simple_pd <- renderUI({
    ...
    sliderInput("pram_simple_pd",
      min = 0.01, max = 1.00, step = 0.01, value = 0.8,
  ...
  output$pram_simple_alpha <- renderUI({
    ...
    sliderInput("pram_simple_alpha",
      min = 0.01, max = 1.00, step = 0.01, value = 0.5,
...
output$ui_kAnon <- renderUI({
  ...
  output$kanon_use_combs <- renderUI({
    ...
    radioButtons("rb_kanon_useCombs",
      choices = c("No", "Yes"), width = "100%", inline = TRUE,
...
output$ui_supp_threshold <- renderUI({
  output$ui_supp_th <- renderUI({
    ...
    sl <- sliderInput("sl_supp_threshold",
      label = p("Set threshold for individual risk", tipify(icon("circle-info"), title = txt_tooltip, placement = "top")),
