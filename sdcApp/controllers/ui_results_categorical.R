...
output$ui_rescat_riskinfo <- renderUI({
  output$ui_rescat_selection <- renderUI({
    radioButtons("rb_riskselection",
      label = p("What kind of results do you want to show?"),
  ...
  output$rescat_riskymeasures <- renderUI({
    ...
    out <- fluidRow(
      ...
      column(12, p(
        code(rI$s), "observations have a higher risk than the risk in the main part of the data, as compared to", code(rI$sorig), "observations in the original data",
    ...
    if (rI$hierrisk) {
      out <- list(out, fluidRow(
        ...
        column(12, p(
          "If cluster information is taken into account, we expect", code(rI$hier_exp_m),
          "(", code(paste0(rI$hier_exp_mp, "%")), ") re-identifications in the anonymized data set. In the original dataset we expected",
          code(rI$hier_exp_o), "(", code(paste0(rI$hier_exp_op, "%")), ") re-identifications."
        ), align = "center")
      ))
    }
    out
  ...
  output$tab_recodes <- DT::renderDataTable(
    {
      ...
      res_o <- lapply(kV_o, function(x) {
        c(nr = length(unique(x)), mean = mean(table(x)), min = min(table(x)))
      })
      res_m <- lapply(kV_m, function(x) {
        c(nr = length(unique(x)), mean = mean(table(x)), min = min(table(x)))
      })
      k <- ncol(kV_o)
      out <- NULL
...
output$ui_bivariate_tab <- renderUI({
  ...
  output$biv_tab_res <- renderPrint({
    ...
    if (!is.null(input$sel_catvar2) & input$sel_catvar2 == "none") {
      ...
      tabs <- fluidRow(
        column(12, h4("Original data"), align = "center"),
