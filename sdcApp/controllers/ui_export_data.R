...
output$ui_export_data <- renderUI({
  output$dt_exportData <- DT::renderDataTable(
    ...
    options = list(scrollX = TRUE, lengthMenu = list(c(10, 20, 50, 100, -1), c("10", "20", "50", "100", "All")), pageLength = 10),
    rownames = FALSE
  )
  ...
  output$ui_export_stata <- renderUI({
    ...
    rb1 <- radioButtons("export_dta_version",
      label = p("To which version of STATA would you like to export?"),
      choices = c(
        "Version 8" = 8, "Version 9" = 9, "Version 10" = 10, "Version 11" = 11,
        "Version 12" = 12, "Version 13" = 13, "Version 14" = 14
      ), inline = TRUE
    )
  ...
  if (!is.null(input$dat_exp_type)) {
    ...
    output$rb_randomize_export <- renderUI({
      ...
      choices <- c(
        "No randomization" = "no", "Randomization at record level" = "simple",
