output$tabinfo_sb_results <- output$tabinfo_sb_anonymize <- renderUI({
  ...
  fluidRow(
    ...
    column(12, DT::renderDataTable(
      ...
      rownames = FALSE,
      colnames = c("Variable name", "Type", "Additional suppressions by local suppression algorithm"),
      selection = "none",
      style = "bootstrap",
...
output$numrisk_sb_anonymize <- renderUI({
  ...
  fluidRow(
    ...
    column(12, DT::renderDataTable(
      ...
      rownames = FALSE,
      colnames = c("Data", "Minimum risk", "Maximum risk"),
      selection = "none",
      style = "bootstrap",
      class = "table-condensed",
      options = list(searching = FALSE, scrollX = TRUE, paging = FALSE, ordering = FALSE, bInfo = FALSE)
...
output$pram_sb_anonymize <- renderUI({
  ...
  out <- list(out, fluidRow(
    column(12, DT::renderDataTable(
      ...
      rownames = FALSE,
      colnames = c("Variable name", "Number of changed values", "Percentage of changed values"),
      selection = "none",
      style = "bootstrap",
      class = "table-condensed",
      options = list(searching = FALSE, scrollX = TRUE, paging = FALSE, ordering = FALSE, bInfo = FALSE)
