...
output$ui_modify_recode_to_factor <- renderUI({
  ...
  output$ui_globalRecode_split <- renderUI({
    radioButtons("sel_custom_split",
      label = p("Use custom breaks?"), choices = c("no", "yes"),
  ...
  output$ui_globalRecode_cutalgo <- renderUI({
    ...
    selectInput("sel_algo",
      label = p("Select algorithm", tipify(icon("circle-info"), title = txt, placement = "top")),
...
output$ui_modify_change_factor <- renderUI({
  ...
  output$reclocfac_levs <- renderUI({
    selectInput("cbg_factor",
      label = p("Select Levels to recode/combine"),
  ...
  output$reclocfac_txtval <- renderUI({
    req(input$cbg_factor)
    txtval <- textInput("inp_newlevname",
      label = p("New label for recoded values"),
...
output$ui_modify_create_stratvar <- renderUI({
  output$sel_genstrata <- renderUI({
    selectInput("sel_allvars_strata",
      label = p("Select variables to generate a stratification variable"), multiple = TRUE,
...
output$ui_set_to_na <- renderUI({
  output$ui_nasupptype <- renderUI({
    radioButtons("set_to_na_type",
      label = p("How to select values to recode to NA?"),
...
output$ui_reset_var <- renderUI({
  output$reset_microvar_var <- renderUI({
    selectInput("sel_reset_microvars",
      label = p("Choose variable(s) to reset"),
...
output$ui_show_microdata <- renderUI({
  ...
  out <- fluidRow(
    column(width = 12, offset = 0, h3("Loaded microdata")),
    class = "wb-header"
  )
  if (is.null(attr(obj$inputdata, "dropped"))) {
    out <- list(out, fluidRow(
      column(width = 12, offset = 0, p(HTML(txt_microdata))),
      class = "wb-header-hint"
    ))
  } else {
    out <- list(out, fluidRow(
      column(width = 8, offset = 2, list(HTML(txt_microdata), code(lapply(attr(obj$inputdata, "dropped"), function(x) {
        x
      }))))
    ))
  }
...
output$ui_sample_microdata <- renderUI({
  sel1 <- selectInput("sel_sdcP_sample_type",
    label = p("Select a method to restrict the number of records"),
    choices = c(
      "n percent of the data" = "n_perc",
  ...
  if (!is.null(input$sel_sdcP_sample_type)) {
    ...
    sl1 <- sliderInput("sel_sdcP_sample_n",
      label = p("Set 'n' for the selected method"),
...
output$ui_hierarchical_data_prep <- renderUI({
  ...
  output$sel_hhvars_id <- renderUI({
    selectInput("sel_hhvars_id",
      label = p("Select the household id variable"),
      choices = c("", allVars()), multiple = FALSE, width = "75%"
    )
  })
  output$sel_hhvars <- renderUI({
    req(input$sel_hhvars_id)
    selectInput("sel_hhvars",
      label = p("Please select all variables that refer to households and not to individuals"),
...
output$ui_hierarchical_data_merge <- renderUI({
  output$sel_hhid_merge <- renderUI({
    selectInput("sel_hhid_hhdata",
      label = p("Select a variable containing household ids"), choices = intersect(colnames(obj$hhdata), colnames(inputdata())),
...
output$ui_hierarchical_data <- renderUI({
  rb1 <- radioButtons("rb_hierdata_selection",
    label = p("What do you want to do?"),
