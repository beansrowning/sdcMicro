...
output$ui_topbotcoding_num <- renderUI({
  ...
  output$ui_topbot_params_num <- renderUI({
    ...
    sel_kind <- radioButtons("sel_topbot_kind_num",
      choices = c("top", "bottom"),
      label = p("Apply top/bottom coding?", tipify(icon("circle-info"), title = txt_tooltip1, placement = "top")), inline = TRUE
    )
    txt_val <- textInput("num_topbot_val_num",
      label = p("Set threshold value", tipify(icon("circle-info"), title = txt_tooltip2, placement = "top")),
      placeholder = "Please enter a number", width = "75%"
    )
    txt_replace <- textInput("num_topbot_replacement_num",
      label = p("Set replacement value", tipify(icon("circle-info"), title = txt_tooltip3, placement = "top")),
      placeholder = "Please enter a number", width = "75%"
    )
...
output$ui_microaggregation <- renderUI({
  ...
  output$ui_microagg_vars <- renderUI({
    ...
    selectInput("sel_microagg_v",
      choices = possvars_numericmethods(),
...
output$ui_noise <- renderUI({
  ...
  output$ui_noise_vars <- renderUI({
    ...
    selectInput("sel_noise_v",
      choices = get_numVars_names(),
