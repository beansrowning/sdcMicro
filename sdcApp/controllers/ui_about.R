output$credits <- renderUI({
  ...
  out <- list(out, fluidRow(
    ...
    column(width = 8, offset = 2, p(
      "We also want to thank", tags$strong("Olivier Dupriez"), "and", tags$strong("Matthew Welch"), "for the possibility to
...
output$stop_btn <- renderUI({
  btn <- bsButton("stop_sdcApp",
    label = "Stop the GUI", style = "primary", size = "extra-small",
...
output$ui_about <- renderUI({
  ...
  out <- fluidRow(
    ...
    column(width = 8, offset = 2, p(
      "This graphical user interface of", code("sdcMicro"), " allows you to anonymize microdata even if you are not an
  ...
  out <- list(out, fluidRow(
    ...
    column(width = 8, offset = 2, p(
      "In case you have any suggestions or bug reports, please file an issue at the",
