...
output$ui_sdcObj_summary <- renderUI({
  output$show_info_general <- renderUI({
    ...
    out <- fluidRow(
      ...
      column(12, list("Categorical key variable(s):", lapply(x$keyVars, function(x) {
        code(x)
      })))
    )
    ...
    if (length(x$numVars) > 0) {
      out <- list(out, fluidRow(
        column(12, list("Numerical key variable(s):", lapply(x$numVars, function(x) {
          code(x)
        })))
      ))
    }
    if (length(x$weightVar) > 0) {
      out <- list(out, fluidRow(
        column(12, list("Sampling weight:", lapply(x$weightVar, function(x) {
          code(x)
        })))
      ))
    }
    ...
    if (length(x$householdId) > 0) {
      out <- list(out, fluidRow(
        column(12, list("Hierarchical identifier:", lapply(x$householdId, function(x) {
          code(x)
        })))
      ))
    }
    if (length(x$delVars) > 0) {
      out <- list(out, fluidRow(
        column(12, list("Deleted variable(s):", lapply(x$delVars, function(x) {
          code(x)
        })))
      ))
    }
      ...
      for (i in 1:length(gV)) {
        out <- list(out, fluidRow(
          column(12, list("Variable(s)", lapply(gV[[i]][[2]], function(x) {
            code(x)
          }), "are linked to key variable", code(gV[[i]][[1]])))
        ))
      }
    }
    out <- list(out, fluidRow(
  ...
  output$show_info_kanon <- renderUI({
    ...
    dt <- data.table(
      ...
      "Modified data" = c(
        paste0(x[["2anon"]]$mod, " (", x[["2anon"]]$mod_p, "%)"),
        paste0(x[["3anon"]]$mod, " (", x[["3anon"]]$mod_p, "%)"),
        paste0(x[["5anon"]]$mod, " (", x[["5anon"]]$mod_p, "%)")
      ),
      "Original data" = c(
        paste0(x[["2anon"]]$orig, " (", x[["2anon"]]$orig_p, "%)"),
  ...
  output$show_info_catrisk <- renderUI({
    ...
    out <- fluidRow(
      ...
      column(12, p(
        "We expect", code(reident$mod), "(", code(paste0(reident$mod_p, "%")), ") re-identifications in the population, as compared to",
        code(reident$orig), "(", code(paste0(reident$orig_p, "%")), ") re-identifications in the original data."
      )),
      column(12, p(
        code(riskyobs$mod), " observations have a higher risk than the risk in the main part of the data, as compared to ",
  ...
  output$show_info_localsuppression <- renderUI({
    ...
    setnames(dt, c(
      "Key variable", "Number of suppressions",
...
output$ui_sdcObj_explorevars <- renderUI({
  ...
  output$view_summary_anon <- renderUI({
    ...
    nainfo$nr_na <- as.integer(unlist(lapply(df, function(x) {
      sum(is.na(x))
    })))
...
output$ui_sdcObj_randIds <- renderUI({
  ...
  output$randid_withinvar <- renderUI({
    ...
    selectInput("sel_randid_withinvar",
      label = p("Set grouping variable (optional)", tipify(icon("circle-info"), title = txt_tooltip, placement = "top")),
...
sdcData <- reactive({
  ...
  df$nrCodes <- sapply(inputdata, function(x) {
    length(unique(x))
  })
  df$nrNA <- sapply(inputdata, function(x) {
    sum(is.na(x))
  })
...
output$setupbtn <- renderUI({
  ...
  if (length(ii) > 0) {
    showBtn <- FALSE
    txt <- p(
      "Categorical key variables have to be of type", dQuote("factor"), " or type ", dQuote("integer"), ".", tags$br(), tags$br(),
  ...
  if (length(ii) > 0) {
    showBtn <- FALSE
    txt <- p(
      "Continuous key variables have to be of type ", dQuote("numeric"), " or type ", dQuote("integer"), tags$br(), tags$br(),
      tags$span(style = "color:red; font-weight:bold", "Change the variable selection or go back to the Microdata tab and change the variable type before making other variable selections.")
    )
    showModal(modalDialog(list(txt), title = strong(paste("Invalid variable choice for ", dQuote(vnames[ii]))), footer = modalButton("Continue"), size = "m", easyClose = TRUE, fade = TRUE), session = session)
  }
  ...
  if (length(ii) > 0) {
    ...
    if (any(useAsKeys[ii] %in% c("Cat.", "Cont."))) {
      showBtn <- FALSE
      txt <- p(
        "Selected pram variables are also key variables.", tags$br(), tags$br(),
    ...
    if (any(useAsWeight[ii] == TRUE)) {
      showBtn <- FALSE
      txt <- p(
        "The selected PRAM variable is also selected as weight variable.", tags$br(), tags$br(),
    ...
    if (any(useAsClusterID[ii] == TRUE)) {
      showBtn <- FALSE
      txt <- p(
        "The selected PRAM variable is also selected as hierarchical identifier.", tags$br(), tags$br(),
    ...
    if (length(kk) > 0) {
      showBtn <- FALSE
      txt <- p(
        "Pram variables have to be of type ", dQuote("factor"), ".", tags$br(), tags$br(),
        tags$span(style = "color:red; font-weight:bold", "Change the variable selection or go back to the Microdata tab and change the variable type before making other variable selections.")
      )
      showModal(modalDialog(list(txt), title = strong(paste("Invalid variable choice (", dQuote(vnames[ii]), ")")), footer = modalButton("Continue"), size = "m", easyClose = TRUE, fade = TRUE), session = session)
    }
  ...
  if (length(ii) > 1) {
    showBtn <- FALSE
    txt <- p(
      "More than one weight variable is selected.", tags$br(), tags$br(),
  ...
  if (length(ii) == 1) {
    ...
    if (useAsKeys[ii] != "No") {
      showBtn <- FALSE
      txt <- p(
        "The selected weight variable is also selected as key variable.", tags$br(), tags$br(),
    ...
    if (!types[ii] %in% c("numeric", "integer")) {
      showBtn <- FALSE
      txt <- p(
        "The weight variable has to be of type ", dQuote("numeric"), " or type ", dQuote("integer"), ".", tags$br(), tags$br(),
        tags$span(style = "color:red; font-weight:bold", "Change the variable selection or go back to the Microdata tab and change the variable type before making other variable selections.")
      )
      showModal(modalDialog(list(txt), title = strong(paste("Invalid variable choice (", dQuote(vnames[ii]), ")")), footer = modalButton("Continue"), size = "m", easyClose = TRUE, fade = TRUE), session = session)
    }
    ...
    if (any(is.na(inputdata()[[vnames[ii]]]))) {
      showBtn <- FALSE
      txt <- p(
        "The weight variable contains", sum(is.na(inputdata()[[vnames[ii]]])), "missing value(s) (NA).", tags$br(), tags$br(),
  ...
  if (length(ii) > 1) {
    showBtn <- FALSE
    txt <- p(
      "More than one hierarchical identifier is selected.", tags$br(), tags$br(),
  ...
  if (length(ii) > 0) {
    ...
    if (length(zz) > 0) {
      showBtn <- FALSE
      txt <- p(
        "The variable selected to be deleted is also selected as key variable.", tags$br(), tags$br(),
    ...
    if (length(zz) > 0) {
      showBtn <- FALSE
      txt <- p(
        "The variable selected to be deleted is also selected as PRAM variable.", tags$br(), tags$br(),
    ...
    if (length(zz) > 0) {
      showBtn <- FALSE
      txt <- p(
        "The variable selected to be deleted is also selected as weight variable.", tags$br(), tags$br(),
    ...
    if (length(zz) > 0) {
      showBtn <- FALSE
      txt <- p(
        "Selected variable to be deleted is also selected as hierarchical identifier.", tags$br(), tags$br(),
